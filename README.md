...
- **Frontend**: one `web/index.html` with vendored CodeMirror and SQL formatter
  bundles, results table/JSON views, saved queries, and CSV export. Embedded into the
  binary via `go:embed`.
...
The UI compresses the selected database, table, tab, filters, sort, pagination,
...
Keyboard: **Ctrl/Cmd + Enter** runs the selection or statement at the cursor.
