...
| SQL preview and count | capped preview rows, exact count without rows, trailing comments/terminators, unsupported EXPLAIN count, stale responses, invalid JSON, DB errors | `internal/db/pool_test.go`, `internal/guard/guard_test.go`, `internal/server/query_handlers_test.go`, `web/app.test.js`, `web/db-sql.test.js` |
...
| SQL editor autocomplete | CodeMirror mount, keymap run, schema/field autocomplete, bounded and cancellable metadata loading, fallback textarea mode, duplicate table names | `web/app.test.js`, `web/codemirror6-entry.test.js`, `web/sql-tab-concurrency.test.js` |
